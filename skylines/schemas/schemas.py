...
class ClubSchema(Schema):
    ...
    name = fields.String(
        required=True,
        strip=True,
        validate=(validate.NotEmpty(), validate.Length(min=1, max=255)),
    )
...
class UserSchema(Schema):
    ...
    trackingCallsign = fields.String(
        attribute="tracking_callsign", strip=True, validate=validate.Length(max=5)
    )
    trackingDelay = fields.Integer(
        attribute="tracking_delay", validate=validate.Range(min=0, max=60)
    )
...
class CurrentUserSchema(UserSchema):
    ...
    distanceUnit = fields.Integer(
        attribute="distance_unit",
        validate=validate.Range(min=0, max=len(DISTANCE_UNITS) - 1),
    )
    speedUnit = fields.Integer(
        attribute="speed_unit", validate=validate.Range(min=0, max=len(SPEED_UNITS) - 1)
    )
    liftUnit = fields.Integer(
        attribute="lift_unit", validate=validate.Range(min=0, max=len(LIFT_UNITS) - 1)
    )
    altitudeUnit = fields.Integer(
        attribute="altitude_unit",
        validate=validate.Range(min=0, max=len(ALTITUDE_UNITS) - 1),
    )
...
class FlightSchema(Schema):
    ...
    copilotName = fields.String(
        attribute="co_pilot_name",
        strip=True,
        allow_none=True,
        validate=validate.Length(max=255),
    )
    ...
    registration = fields.String(
        allow_none=True, strip=True, validate=validate.Length(max=32)
    )
    competitionId = fields.String(
    ...
    landingAirport = fields.Nested(
        AirportSchema, attribute="landing_airport", only=("id", "name", "countryCode")
    )
...
class FlightPhaseSchema(Schema):
    circlingDirection = fields.Function(
        lambda phase: CIRCDIR_IDS.get(phase.circling_direction)
    )
