...
def latest():
    ...
    query = (
        db.session.query(func.max(Flight.date_local).label("date"))
...
def unassigned():
    ...
    return _create_list(
        filter=f, default_sorting_column="date", default_sorting_order="desc"
    )
...
def read(flight_id):
    ...
    circling_performance = circling_performance_schema.dump(
        flight.circling_performance, many=True
    ).data
...
def json(flight_id):
    ...
    igc_file = '/home/bret/skylinesC/htdocs/files/{}'.format(flight.igc_file.filename)
    ...
    for flight in result:
        ...
        if closest == 0 or closest == len(trace) - 1:
            ...
            dx = (time - flight.timestamps[next_smaller]).total_seconds() / (
                flight.timestamps[next_larger] - flight.timestamps[next_smaller]
            ).total_seconds()
...
@flights_blueprint.route(
    "/flights/<flight_id>", methods=("DELETE",), strict_slashes=False
)
@oauth.required()
