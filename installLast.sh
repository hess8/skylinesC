...
sudo pipenv install --verbose --dev  #--skip-lock #this doesn't work with skip-lock...seems to work without it, but can't lock anyway due to oauth dependency problems

create PostGIS databases

sudo sudo -u postgres createuser -s $USER

sudo sudo -u postgres createdb skylines2 -O $USER
sudo sudo -u postgres psql -d skylines2 -c 'CREATE EXTENSION postgis;'
sudo sudo -u postgres psql -d skylines2 -c 'CREATE EXTENSION fuzzystrmatch;'
